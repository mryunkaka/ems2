...
function forensicMedicStatusMeta(array $user): array
{
...
function forensicMedicPromotionDate(array $user): ?string
{
...
function forensicMedicTenureDays(?string $date): int
{
    if (!$date) {
        return 0;
    }
...
function forensicMedicDocStatus(?string $path, string $requiredLabel = 'Completed', string $missingLabel = 'Not Yet'): array
{
...
function forensicMedicMedicalClassMeta(array $user): array
{
    $position = ems_normalize_position($user['position'] ?? '');
    ...
    if ($position === 'paramedic') {
        return forensicMedicDocStatus($user['sertifikat_class_paramedic'] ?? null);
    }

    return forensicMedicDocStatus($user['sertifikat_class_co_asst'] ?? null);
}

function forensicMedicCertificateRequirements(array $user): array
{
    $position = ems_normalize_position($user['position'] ?? '');
...
function forensicMedicCertificateSummary(array $user): array
{
    $requirements = forensicMedicCertificateRequirements($user);
    $missing = [];
...
try {
    ...
    foreach ($rows as $row) {
        $statusMeta = forensicMedicStatusMeta($row);
        $promotionDate = forensicMedicPromotionDate($row);
        $tenureDays = forensicMedicTenureDays($promotionDate);
        ...
        if ($certificateSummary['label'] === 'Lengkap') {
            $summary['complete']++;
        }
    }
} catch (Throwable $e) {
    $errors[] = 'Gagal memuat daftar medis forensic: ' . $e->getMessage();
}
...
<section class="content">
    <div class="page page-shell">
        <div class="forensic-medics-hero mb-4">
            <div>
                <div class="forensic-medics-kicker">Forensic Monitoring</div>
        ...
        <div class="stats-grid mb-4">
            ...
            ems_component('ui/statistic-card', ['label' => 'Sertifikat Lengkap', 'value' => number_format($summary['complete']), 'icon' => 'shield-check', 'tone' => 'info']);
            ?>
        </div>
            ...
            <div class="card-body">
                <div class="forensic-medics-tabs">
                    <?php foreach ($allowedPositionFilters as $value => $label): ?>
                        <?php
                        $href = $value === ''
                            ? ems_url('/dashboard/forensic_medics.php')
                            : ems_url('/dashboard/forensic_medics.php?position=' . urlencode($value));
                        ?>
        ...
        <div class="card card-section">
            <div class="card-header card-toolbar">
                <span>Daftar Medis</span>
                <div class="toolbar-group">
                    <?php
                    $exportUrl = $positionFilter === ''
                        ? ems_url('/dashboard/forensic_medics_export.php')
                        : ems_url('/dashboard/forensic_medics_export.php?position=' . urlencode($positionFilter));
                    ?>
            ...
            <p class="meta-text mb-4">Status `Available`, `On Leave`, dan `Resigned` dihitung dari data aktif/cuti/resign user. Status sertifikat memakai upload dokumen yang sudah ada tanpa tabel baru.</p>

            <div class="table-wrapper">
                <table id="forensicMedicsTable" class="table-custom w-full">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Name</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="meta-text-xs">
                                        <?= $medic['batch'] > 0 ? 'Batch ' . (int)$medic['batch'] : 'Tanpa Batch' ?>
                                    </div>
                                </td>
                                ...
                                <td data-order="<?= (int)$medic['status_meta']['sort'] ?>">
                                    ...
                                    <div class="meta-text-xs"><?= htmlspecialchars((string)$medic['status_meta']['detail'], ENT_QUOTES, 'UTF-8') ?></div>
                                </td>
                                ...
                                <td data-order="<?= (int)$medic['plastic_meta']['sort'] ?>">
                                    <span class="<?= htmlspecialchars($medic['plastic_meta']['class'], ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars($medic['plastic_meta']['label'], ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                </td>
                                <td data-order="<?= (int)$medic['minor_meta']['sort'] ?>">
                                    <span class="<?= htmlspecialchars($medic['minor_meta']['class'], ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars($medic['minor_meta']['label'], ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                </td>
                                <td data-order="<?= (int)$medic['major_meta']['sort'] ?>">
                                ...
                                <td data-order="<?= (int)$medic['medical_class_meta']['sort'] ?>">
                                    <span class="<?= htmlspecialchars($medic['medical_class_meta']['class'], ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars($medic['medical_class_meta']['label'], ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                </td>
                                <td data-order="<?= (int)$medic['certificate_summary']['sort'] ?>">
                                    <span class="<?= htmlspecialchars($medic['certificate_summary']['class'], ENT_QUOTES, 'UTF-8') ?>">
                                        <?= htmlspecialchars($medic['certificate_summary']['label'], ENT_QUOTES, 'UTF-8') ?>
                                    </span>
                                    <div class="meta-text-xs"><?= htmlspecialchars((string)$medic['certificate_summary']['detail'], ENT_QUOTES, 'UTF-8') ?></div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
